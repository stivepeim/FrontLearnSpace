var formidable = require("formidable"),         // 引用formidable模块，用于文件保存
    ...
    url = require("url"),                       //  url模块
    crypto = require('crypto');                  // 加密解密模块
...
http.createServer(function (req, res) {         // 创建新服务器
    ...
    if (req.method == 'OPTIONS') {
        ...
        form.parse(req, function (err) {            // 调用parse接口解析请求信息并保存文件
            util.pump(fs.createReadStream(single_file.path),
                      fs.createWriteStream(__dirname + '\\images\\' + file_name), function (err) {
                          if (err) {
                              console.log(err);
                          }
                      });
