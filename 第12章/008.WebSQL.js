    ...
    function add_item(data, toDB) {                                 // 添加列表项、数据库
        if (toDB) {
            storageDriver.transaction(function (t) {                // 往数据库插入一条数据
                t.executeSql("INSERT INTO " + DB_NAME + " (name,sex,age) VALUES (?,?,?);",
                    [data.name, data.sex, data.age],                // 传入保存数据
                    function (transaction, resultSet) {
                        data.id = resultSet.insertId;               // 获取数据库返回的自增ID
                        tbody.append(substitute(item_tpl, data));   // 添加到列表
                    }, function () { alert('添加失败'); });         // 错误回调函数
            });
        } else { tbody.append(substitute(item_tpl, data)); };
    };
    function del_item(target) {                                     // 删除一条数据
        storageDriver.transaction(function (t) {                    // 执行删除SQL
            t.executeSql("delete from " + DB_NAME + " where id=" + target.attr('data-id'),
                [], function () {
    ...
    function modify_item(target) {
        ...
        function update_db(data) {
            storageDriver.transaction(function (t) {
                t.executeSql("update " + DB_NAME + " set name=?, sex=?, age=? where id=?",
                    [data.name, data.sex, data.age, id],                // 传入保存数据
                    function (transaction, resultSet) {
        ...
        storageDriver.transaction(function (t) {
            t.executeSql("SELECT * FROM " + DB_NAME + ' where id=' + id, [],                // 读物数据表
                function (t, results) {
